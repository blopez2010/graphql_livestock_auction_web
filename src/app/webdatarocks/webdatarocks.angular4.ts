...
export class WebDataRocksPivot {
  ...
  constructor(el: ElementRef, private dataReportService: DataReportService) {
    this.root = <HTMLElement>el.nativeElement;
  }

  // tslint:disable-next-line: use-life-cycle-interface
  public ngOnInit() {
    this.subscription = this.dataReportService.updateState.subscribe((data: any) => {
      this.webDataRocks.setReport(data.data);
      if (data.options) {
        this.webDataRocks.setOptions(data.options);
