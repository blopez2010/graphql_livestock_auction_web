...
export class AuctionComponent implements OnInit {
  ...
  private initForm() {
    ...
    this.auctionForm
      ...
      .valueChanges.subscribe(value => {
        if (value) {
          this.itemsService
            .getByOrdinal(value, this.route.data['value']['activeEvent'].id)
            .subscribe(
              (result: Response) => {
  ...
  private getTotalBuying() {
    this.transactionService
      .getTotalAmountByEvent(this.route.data['value']['activeEvent'].id)
      .subscribe(result => {
        this.auctionForm
          .get('transaction')
          .get('totalBuying')
    ...
    this.auctionForm
      .get('transaction')
      .get('totalCount')
      .setValue(this.totalCount - this.countDown);
    console.log(this.totalCount - this.countDown);
  }

  private getItemsCountDown() {
    this.itemsService
      .getItemsCountDown(this.route.data['value']['activeEvent'].id)
      .subscribe(result => {
  ...
  private getTotalCount() {
    this.itemsService
      .getTotalItems(this.route.data['value']['activeEvent'].id)
      .subscribe(result => {
  ...
  public addPeople(data: People) {
    this.peopleService.create(data).subscribe((result: Response) => {
      ...
      this.selectedBuyer = {
        ...result.data,
        fullName: result.data.nickname
          ? `${result.data.name} (${result.data.nickname})`
    ...
    if (this.auctionForm.valid && this.selectedBuyer) {
      this.transactionService
        .donate({
          ...
          description: this.auctionForm.get('transaction').get('description')
            .value,
